...
class Compiler
  ...
  def prepare_enclose_io_vars
    @utils.chdir(@ruby_source_dir) do
      ...
      File.open('enclose_io.h', 'w') do |f|
        ...
        f.puts ''
        f.puts '#define RUBY_PACKER_RUBYC_BUILD_PASS2 1'
        f.puts "#define RUBY_PACKER_ENTRANCE #{@memfs_entrance.inspect}" if @entrance
        f.puts '#define RUBY_PACKER_GEMFILE 1' if @root_gemfile_exists
  ...
  def compile_env
    if Gem.win_platform?
      {
        'CI' => 'true',
        'RUBY_PACKER_USE_ORIGINAL_RUBY' => 'true',
        'MAKE' => 'nmake',
        'CL' => '/MP'
      }
    else
  ...
  def compile_pass1_env
    compile_env.merge({
                        'RUBY_PACKER_RUBYC_1ST_PASS' => 'true',
  ...
  def compile_pass2_env
    compile_env.merge({
                        'RUBY_PACKER_RUBYC_1ST_PASS' => nil,
                        'RUBY_PACKER_RUBYC_2ND_PASS' => 'true'
                      })
  end
